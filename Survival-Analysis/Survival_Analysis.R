...
fit <- survfit(Surv(time, censor) ~ group, data = test_s)
...
###deviance residuals:http://www.sthda.com/english/wiki/cox-model-assumptions
#To test influential observations or outliers
res.cox2 <- coxph(Surv(time, censor) ~ group, data = test_s)
